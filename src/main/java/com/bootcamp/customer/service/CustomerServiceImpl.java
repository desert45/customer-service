...
public class CustomerServiceImpl implements CustomerService {
  ...
  public Mono<Customer> save(Customer customer) {
    return this.customerRepository.findById(customer.getDocumentNumber())
        .hasElement().flatMap(exist -> {
          if (exist)
            return Mono.error(
                new CustomerCreationException("Customer already exists IF"));
          return customerRepository.save(customer);
        })
        .doOnSuccess(c -> log.info("Created new customer with ID: {}",
            c.getDocumentNumber()))
  ...
  public Mono<Customer> findById(String documentNumber) {
    return this.customerRepository.existsById(documentNumber).flatMap(rta -> {
      if (rta)
  ...
  public Mono<Void> delete(String documentNumber) {
    return this.customerRepository.findById(documentNumber).flatMap(
        existingCustomer -> customerRepository.delete(existingCustomer));
  }
